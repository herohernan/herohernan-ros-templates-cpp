...
#define     NODE_NAME       "arduino_volt2temp" 
#define     LOOP_RATE       2                       // 2Hz                  
#define     TOPIC1_SUB      "arduino_AI_volt"       // Sub = Subscribe [0.0 - 5.0 V]
#define     TOPIC1_PUB      "arduino_AI_temp"       // Pub = Publisher [0 - 300 °C]
...
int main(int argc, char **argv)
{
    ...
    // Loop ROS
    std_msgs::Float32 sensorValue_temp;
    while(ros::ok()) 
    {
        // Conversion
        sensorValue_temp.data = sensorValue_volt.data * (300.0 / 5.0);
		ROS_INFO("Sensor value got from Arduino was converted from %f [Volt] to %f [°C]", sensorValue_volt.data, sensorValue_temp.data);

	    // Publishing
        topic1_pub.publish(sensorValue_temp);
